...
In above diagram, for branches `main` and `ip_parsing`, commit `B` is the "most common ancestor", or might also be called the "merge base", since when merging the branches, the commits after `B` on both branches need to be handled, i.e, check whether `D`, `E` interfere with the work done by `C` or vice-versa.
